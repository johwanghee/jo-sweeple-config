...
#define SPLIT_BLE_PERIPHERAL_COUNT 2

static sys_slist_t widgets = SYS_SLIST_STATIC_INIT(&widgets);

struct battery_status_state {
    uint8_t level[SPLIT_BLE_PERIPHERAL_COUNT];
} battery_state;
...
struct battery_widget_object battery_widget_objects[SPLIT_BLE_PERIPHERAL_COUNT];
...
static void set_battery_symbol(lv_obj_t *widged, struct battery_status_state state) {
    ...
    for (int i = 0; i < SPLIT_BLE_PERIPHERAL_COUNT; i++) {
